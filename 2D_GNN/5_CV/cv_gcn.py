...
class GCN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=5, dropout=0.2):
...
for fold, (train_idx, test_idx) in enumerate(kfold.split(graph_list)):
    ...
    train_loader = DataLoader(graph_list, batch_size=64, sampler=SubsetRandomSampler(train_idx))
    test_loader = DataLoader(graph_list, batch_size=64, sampler=SubsetRandomSampler(test_idx))
    """
    model = GCN(
        in_channels=9,
        hidden_channels=192,
