...
for i, smile in enumerate(df['smiles']):
    g = from_smiles(smile)  # requires PyG >= 2.3.0
    g.x = g.x.float()
...
# Custom dataset class (optional here, provided for completeness)
class MyOwnDataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        super().__init__(root, transform, pre_transform, pre_filter)
        self.load(self.processed_paths[0])
    @property
    def raw_file_names(self): return 'data.csv'
    @property
    def processed_file_names(self): return 'data.dt'
    def download(self): pass
    def process(self):  # Not used here directly
...
# -------------------------------
# GCN MODEL DEFINITION (replace GIN)
# -------------------------------
...
# Model and optimizer (with your requested hyperparameters)
...
# Evaluation function (same as your original, works for both GIN & GCN)
@torch.no_grad()
def evaluate(loader):
    model.eval()
...
plt.subplot(1, 2, 1)
plt.scatter(test_results['actual_log'], test_results['pred_log'], color='blue', label='Test', edgecolors='black', s=50)
sns.regplot(data=train_results, x='actual_log', y='pred_log', color='red',
...
anchored_text = AnchoredText(f"R² (Train): {train_r2_log:.4f}\nR² (Test): {test_r2_log:.4f}", loc='upper left', prop=dict(size=18, weight='bold'))
plt.gca().add_artist(anchored_text)
plt.grid(True, linestyle='--', alpha=0.5)
plt.subplot(1, 2, 2)
plt.scatter(test_results['actual_log'], test_results['actual_log'] - test_results['pred_log'], color='blue', label='Test', edgecolors='black', s=50)
sns.scatterplot(x=train_results['actual_log'], y=train_results['actual_log'] - train_results['pred_log'], color='red', alpha=0.3, edgecolor='black', label='Train')
plt.xlabel('Actual (log)', fontsize=20, fontweight='bold')
...
plt.grid(True, linestyle='--', alpha=0.5)
ax = plt.gca()
ax.text(0.99, 0.03, 'GCN', transform=ax.transAxes, fontsize=18, fontweight='bold', color='black', va='bottom', ha='right', alpha=0.9)
plt.tight_layout()
plt.savefig('plots/act_pred_res_plot_log_hd.png', dpi=600)
plt.close()

plt.figure(figsize=(7, 6), dpi=300)
sns.histplot(test_results['actual_log'] - test_results['pred_log'], kde=True, color='blue', bins=30, edgecolor='black')
...
plt.figure(figsize=(14, 6), dpi=300)
plt.subplot(1, 2, 1)
plt.scatter(test_results['actual_orig'], test_results['pred_orig'], color='blue', label='Test', edgecolors='black', s=50)
sns.regplot(data=train_results, x='actual_orig', y='pred_orig', color='red',
    scatter_kws={'s': 40, 'alpha': 0.3, 'edgecolor': 'black'})
...
plt.yticks(fontsize=16, fontweight='bold')
plt.legend(['Test', 'Train'], frameon=False, loc='lower right', fontsize=20)
anchored_text = AnchoredText(f"R² (Train): {train_r2_orig:.4f}\nR² (Test): {test_r2_orig:.4f}", loc='upper left', prop=dict(size=18, weight='bold'))
plt.gca().add_artist(anchored_text)
plt.grid(True, linestyle='--', alpha=0.5)
plt.subplot(1, 2, 2)
plt.scatter(test_results['actual_orig'], test_results['actual_orig'] - test_results['pred_orig'], color='blue', label='Test', edgecolors='black', s=50)
sns.scatterplot(x=train_results['actual_orig'], y=train_results['actual_orig'] - train_results['pred_orig'], color='red', alpha=0.3, edgecolor='black', label='Train')
plt.xlabel('Actual', fontsize=20, fontweight='bold')
plt.ylabel('Residual (Actual - Predicted)', fontsize=20, fontweight='bold')
plt.xticks(fontsize=16, fontweight='bold')
plt.yticks(fontsize=16, fontweight='bold')
...
plt.figure(figsize=(7, 6), dpi=300)
sns.histplot(test_results['actual_orig'] - test_results['pred_orig'], kde=True, color='blue', bins=30, edgecolor='black')
